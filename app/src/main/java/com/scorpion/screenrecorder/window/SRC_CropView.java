package com.scorpion.screenrecorder.window;
...
import com.scorpion.screenrecorder.window.edge.SRC_Edge;
import com.scorpion.screenrecorder.window.handle.SRC_Handle;
import com.scorpion.screenrecorder.window.util.SRC_AspectRatioUtil;
import com.scorpion.screenrecorder.window.util.SRC_HandleUtil;
import com.scorpion.screenrecorder.window.util.SRC_PaintUtil;


public class SRC_CropView extends View {
    private static final int SNAP_RADIUS_DP = 6;
    private static final float DEFAULT_SHOW_GUIDELINES_LIMIT = 100.0F;
    private static final float DEFAULT_CORNER_THICKNESS_DP = SRC_PaintUtil.getCornerThickness();
    private static final float DEFAULT_LINE_THICKNESS_DP = SRC_PaintUtil.getLineThickness();
    ...
    private SRC_Handle mPressedHandle;
    ...
    public SRC_CropView(Context context) {
        super(context);
    ...
    public SRC_CropView(Context context, AttributeSet attrs) {
        super(context, attrs);
        mTargetAspectRatio = (float)mAspectRatioX / (float)mAspectRatioY;
        initializedCropWindow = false;
        init(context);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawRect(SRC_Edge.LEFT.getCoordinate(), SRC_Edge.TOP.getCoordinate(), SRC_Edge.RIGHT.getCoordinate(), SRC_Edge.BOTTOM.getCoordinate(), mBorderPaint);
        drawCorners(canvas);
    }
    ...
    private void init(Context context) {
        ...
        mHandleRadius = SRC_HandleUtil.getTargetRadius(context);
        mSnapRadius = TypedValue.applyDimension(1, 6.0F, displayMetrics);
        mBorderPaint = SRC_PaintUtil.newBorderPaint(context);
        mGuidelinePaint = SRC_PaintUtil.newGuidelinePaint();
        mBackgroundPaint = SRC_PaintUtil.newBackgroundPaint(context);
        mCornerPaint = SRC_PaintUtil.newCornerPaint(context);
    ...
    private void initCropWindow(Rect bitmapRect) {
        ...
        if (mFixAspectRatio) {
            float halfCropWidth;
            if (SRC_AspectRatioUtil.calculateAspectRatio(bitmapRect) > mTargetAspectRatio) {
                SRC_Edge.TOP.setCoordinate((float)bitmapRect.top);
                SRC_Edge.BOTTOM.setCoordinate((float)bitmapRect.bottom);
                centerX = (float)getWidth() / 2.0F;
                cropWidth = Math.max(40.0F, SRC_AspectRatioUtil.calculateWidth(SRC_Edge.TOP.getCoordinate(), SRC_Edge.BOTTOM.getCoordinate(), mTargetAspectRatio));
                if (cropWidth == 40.0F) {
                    mTargetAspectRatio = 40.0F / (SRC_Edge.BOTTOM.getCoordinate() - SRC_Edge.TOP.getCoordinate());
                }

                halfCropWidth = cropWidth / 2.0F;
                SRC_Edge.LEFT.setCoordinate(centerX - halfCropWidth);
                SRC_Edge.RIGHT.setCoordinate(centerX + halfCropWidth);
            } else {
                SRC_Edge.LEFT.setCoordinate((float)bitmapRect.left);
                SRC_Edge.RIGHT.setCoordinate((float)bitmapRect.right);
                centerX = (float)getHeight() / 2.0F;
                cropWidth = Math.max(40.0F, SRC_AspectRatioUtil.calculateHeight(SRC_Edge.LEFT.getCoordinate(), SRC_Edge.RIGHT.getCoordinate(), mTargetAspectRatio));
                if (cropWidth == 40.0F) {
                    mTargetAspectRatio = (SRC_Edge.RIGHT.getCoordinate() - SRC_Edge.LEFT.getCoordinate()) / 40.0F;
                }

                halfCropWidth = cropWidth / 2.0F;
                SRC_Edge.TOP.setCoordinate(centerX - halfCropWidth);
                SRC_Edge.BOTTOM.setCoordinate(centerX + halfCropWidth);
            }
        } else {
            centerX = 0.1F * (float)bitmapRect.width();
            cropWidth = 0.1F * (float)bitmapRect.height();
            SRC_Edge.LEFT.setCoordinate((float)bitmapRect.left + centerX);
            SRC_Edge.TOP.setCoordinate((float)bitmapRect.top + cropWidth);
            SRC_Edge.RIGHT.setCoordinate((float)bitmapRect.right - centerX);
            SRC_Edge.BOTTOM.setCoordinate((float)bitmapRect.bottom - cropWidth);
        }

    }

    public static boolean showGuidelines() {
        return Math.abs(SRC_Edge.LEFT.getCoordinate() - SRC_Edge.RIGHT.getCoordinate()) >= 100.0F && Math.abs(SRC_Edge.TOP.getCoordinate() - SRC_Edge.BOTTOM.getCoordinate()) >= 100.0F;
    }

    private void drawRuleOfThirdsGuidelines(Canvas canvas) {
        float left = SRC_Edge.LEFT.getCoordinate();
        float top = SRC_Edge.TOP.getCoordinate();
        float right = SRC_Edge.RIGHT.getCoordinate();
        float bottom = SRC_Edge.BOTTOM.getCoordinate();
        float oneThirdCropWidth = SRC_Edge.getWidth() / 3.0F;
        float x1 = left + oneThirdCropWidth;
        canvas.drawLine(x1, top, x1, bottom, mGuidelinePaint);
        float x2 = right - oneThirdCropWidth;
        canvas.drawLine(x2, top, x2, bottom, mGuidelinePaint);
        float oneThirdCropHeight = SRC_Edge.getHeight() / 3.0F;
    ...
    private void onActionDown(float x, float y) {
        ...
        float right = SRC_Edge.RIGHT.getCoordinate();
        float bottom = SRC_Edge.BOTTOM.getCoordinate();
        mPressedHandle = SRC_HandleUtil.getPressedHandle(x, y, left, top, right, bottom, mHandleRadius);
        if (mPressedHandle != null) {
            mTouchOffset = SRC_HandleUtil.getOffset(mPressedHandle, x, y, left, top, right, bottom);
            invalidate();
        }
    }
